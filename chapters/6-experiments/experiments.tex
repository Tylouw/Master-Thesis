...
First, as the physical success-rate of insertions is dependent on the applied deviations and its magnitude, the goal is to asses which set of deviations, and in particular which level (magnitude), cause more insertion failures than other settings.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lll}
        \toprule
        \textbf{Parameter} & \textbf{Level} & \textbf{Interpretation} \\
        \midrule
...
In order to more easily report the deviations, the levels of each deviation parameter are combined together in semantic meaningful groups. However, before the following calculations are necessary.
...
The tilt deviation is represented by the angular deviation angles $\theta_x$ and $\theta_y$ around the $x,y$-axis. Its magnitude is computed as
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.20\textwidth} p{0.31\textwidth} p{0.4\textwidth}}
        ...
        \texttt{force\_level} 
        & Commanded insertion force level.
        ...
        & \texttt{early\_negative}: $-0.21 \leq t < -0.14~\mathrm{s}$ \newline
          \texttt{0}: $-0.01 \leq t < 0.01~\mathrm{s}$ \newline
          \texttt{mid}: $0.19 \leq t < 0.31~\mathrm{s}$ \newline
          \texttt{late}: $0.49 \leq t < 0.91~\mathrm{s}$ \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.2\textwidth} p{0.25\textwidth} p{0.47\textwidth}}
        ...
        & Combination of commanded force level and force limit. This group is used because the effect of the force limit depends on the commanded force. The difference of these forces is labeled with a \texttt{D}.
        & (\texttt{F15,L30,D15}),(\texttt{F15,L40,D25}),(\texttt{F15,L50,D35}),\newline
        (\texttt{F20,L30,D10}),(\texttt{F20,L40,D20}),(\texttt{F20,L50,D30}),\newline
        (\texttt{F25,L30,D5}),(\texttt{F25,L40,D15}),(\texttt{F25,L50,D25}),\newline
        (\texttt{F30,L30,D0}),(\texttt{F30,L40,D10}),(\texttt{F30,L50,D20}),\newline
        (\texttt{F35,L30,D-5}),(\texttt{F35,L40,D5}),(\texttt{F35,L50,D15}),\newline
        (\texttt{F40,L30,D-10}),(\texttt{F40,L40,D0}),(\texttt{F40,L50,D10}),\\

        \bottomrule
    \end{tabular}
\end{table}

% Define radius, tilt magnitude, direction sectors, force groups, record offset groups.
% Define domain = batch + rod_type + tolerance + gripper_type.
% Explain global vs domain-controlled reporting.
\newpage
The same way the dataset's metadata is grouped by deviations and combined deviations, the dataset's sequences can be grouped by domains and combined domains. A domain is defined as a unique combination of attributes:
...
This way the dataset can be clearly indexed to create different subsets which are then used for further testing.
If for example all attributes are set to \texttt{None}, essentially all are selected, which means the whole dataset is indexed.
But if for instance \texttt{rod\_type = small\_rod}, \texttt{batch = batch\_3}, then the selected subset contains only sequences from \texttt{small\_rod} and \texttt{batch\_3}, across all tolerances and all gripper types. 
