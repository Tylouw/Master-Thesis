...
This chapter defines and explains the experimental structure used in the thesis. It first introduces in Section \ref{sec:reporing_of_experiments} the concept of a domain, which is used to select dataset subsets based on batch, rod type, tolerance, and gripper type. It also describes how benchmark experiments are reported, including the repeated seed-based evaluation procedure and the plot format used to compare performance across runs.

The chapter then presents three experiments. Experiment 1 in Section \ref{sec:exp1_best_channel_study} is the best channel study, which is treated as a separate exploratory study because its result defines the channel configuration used in the remaining thesis. It therefore contains its own motivation, method, results, discussion, and conclusion. Experiment 2 in Section \ref{sec:experiment_2} evaluates the selected PatchTST configurations through three benchmark settings: a mixed-split test, a leave-one-hole-out test, and held-out domain group tests. These benchmarks analyze how performance changes depending on which domain is used as the test set.

Finally, Experiment 3 in Section \ref{sec:experiment_3} investigates the recorded deviations in the dataset. This deviation analysis is divided into DA1, which studies how deviation groups affect the physical insertion success rate, and DA2, which analyzes whether specific deviations lead to systematic model confusion. Together, the experiments define how the dataset is evaluated both as a classification benchmark and as a source for analyzing physical insertion behavior.
...
\begin{table}[htbp]
    \centering
    \caption{Allowed gripper-type combinations by batch and rod type.}
...
\figref{fig:exp_vis} shows visually how the experiments are conducted, what domains and channels are used. Red arrows indicate that all domains are used, meaning the whole dataset. Experiment 1 starts with the Best Channel Study, which uses the whole dataset and asses the best channels below. The FT channel is used as input for the PatchTST Base model, forming the Base config for the later Benchmarks. The best channels are used with the Big PatchTST model to form the Big Config.
Experiment 2 defines the benchmarks:
Benchmark 1 uses a mixed split from the whole dataset as the test set and evaluates it on the Base and Big config.
Benchmark 2 uses exclusively the \texttt{rod\_type}-\texttt{tolerance} combinations as the test set.
Benchmark 3 uses all domains individually as the test set.

Experiment 3 is the Deviation Analysis (DA).
DA1 Physical Success asses how the deviations applied during insertions affect the success rate on a global scale, by using the whole dataset, followed by a domain specific analysis using individual or combined domains.
DA2 Model Confusion asses how the deviations affect classification performance. The input data is here the prediction records coming from Benchmark 1, ran on the Big config. Then the affect of deviations on a global scale and later on a specific domain scale, are analyzed.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.9\textwidth]{chapters/6-experiments/Experiments_figure.png}
    \caption{Visualization of experiments conducted.}
    \label{fig:exp_vis}
\end{figure}
\newpage
...
To keep the search doable, the study was not designed as a full evaluation of all possible channel subsets. Instead, it was structured as a two-stage procedure: first, each channel group was evaluated individually to identify promising signal sources and second, the strongest groups were combined in different ways to determine an effective multi-modal channel configuration.
...
\begin{figure}[htpb]
    ...
    \caption{Overview of the evaluated channel configurations. Crosses indicate which channel groups are included in each configuration. The corresponding validation balanced accuracy and validation loss for the available \texttt{Base} and \texttt{Big} runs are shown on the right. The temperature signal is not shown, because it heavily skews the plot.}
...
\begin{figure}[htbp]
    ...
    \caption{Schematic drawing of the insertion taskboard. The nine hole domains used in Benchmark 2 are labeled from 1 to 9. Each hole corresponds to one specific domain, defined in \tabref{tab:bm2_domains}. 1: \texttt{BM2\_big\_1} $\dots$ 9: \texttt{BM2\_rect\_3}.}
...
\begin{figure}[htbp]
    ...
    \caption{Held-out group domains used in Benchmark 3. In each case, all sequences belonging to the highlighted group are assigned to the test set. 1: \texttt{BM3\_big} $\dots$ 3: \texttt{BM3\_rect}, 4: \texttt{BM3\_tol\_1} $\dots$ 6: \texttt{BM3\_tol\_3}}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.20\textwidth} p{0.31\textwidth} p{0.4\textwidth}}
        ...
        & \texttt{GH0}: Grasp height 0 mm\\
        \texttt{GH7}: Grasp height 7 mm\\

        \midrule

        \texttt{approach\_height} 
        & Height offset used during the approach motion before insertion.
        & \texttt{AH-5}: Approach height -5 mm\\
        \texttt{AH0}: Approach height 0 mm\\
        \texttt{AH2}: Approach height 2 mm\\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{0.2\textwidth} p{0.25\textwidth} p{0.47\textwidth}}
        ...
        (\texttt{F20/L30}), (\texttt{F20/L40}), (\texttt{F20/L50}),\newline
